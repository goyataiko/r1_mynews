                    ...
                    <div class="post">
                        <div class="row">
                            <div class="text col-md-6">
                                <div class="date">
                                    {{ $one_post->updated_at->format('Y年m月d日') }}
                                </div>
                                <div class="title">
                                    {{ Str::limit($one_post->title, 150) }}
                                </div>
                                <div class="body mt-3">
                                    {{ Str::limit($one_post->body, 1500) }}
                                </div>
                            </div>
                            <div class="image col-md-6 text-right mt-4">
                                @if ($one_post->image_path)
                                    <img src="{{ secure_asset('storage/image/' . $one_post->image_path) }}">
                                @endif
                            </div>
                        </div>
                    </div>
